...
/**
 * Servlet implementation class WithdrawHandler
 */
@WebServlet("/WithdrawHandler")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("account")!=null && request.getParameter("amount")!=null && Double.parseDouble(request.getParameter("amount"))>0) {
			...
			if(withdrawStatus > 0) {
				...
				response.sendRedirect("withdraw.jsp?balance=0");
			}else {
				response.sendRedirect("withdraw.jsp?success=0");
			}
		}else {
			response.sendRedirect("withdraw.jsp?success=0");
		}
